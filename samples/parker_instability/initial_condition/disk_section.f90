!  This module generates the initial conditions for simulating a section
!  of
!  a galaxy disk.
!
...
! Parameters
  ...
  real :: Pmag_to_Ptherm = 0.0 ! Ratio between thermal and magneticpressure
  real :: Pcr_to_Ptherm = 0.0 ! Ratio between thermal and cosmic raypressure
  real :: fgas = 0.1           ! Gas fraction (Sigma_g/Sigma_total)
  real :: initial_uu_noise=0.0 ! Amplitude of the noise in uu at themidplane
...
!
      if (lroot) call svn_id( &
         "$Id: noinitial_condition.f90 19193 2012-06-30 12:55:46Zwdobler $")
!
...
!     Computes the recommended initial sound speed (to achieve
!     hydrostatic
!      equilibrium)
...
!
!  Initializes all the f arrays in one call.  This subroutine is called
!  last.
!
...
!
      ...
      real, dimension(mx,my,mz,4) :: imposed_density,imposed_cosmicray
      real, dimension (nx) :: prof1
...
!        del2_prof =
!        2*(A/scaleheight**2)*((cosh(z(n)/scaleheight))**(-4.0)-&
!                            2*(tanh(z(n)/scaleheight)**(2))*((cosh(z(n)/scaleheight))**(-2.0)))
!        del6_prof =
!        (-A/scaleheight**(-6.))*(17*(cosh(z(n)/scaleheight))**(-8.0)-&
...
!            imposed_density(l1:l2,m,n,d6rho_imp) = del6_prof
      ...
      call put_shared_variable('imposed_density',imposed_density)
        ...
        do n=n1,n2
         prof = A*2*cosh(z(n)/scaleheight)**(-1)
!        del2_prof
!        =2*A*scaleheight*(tanh(z(n)/scaleheight/2)*cosh(z(n)/scaleheight/2)**(-2.0)*&
!                    (tanh(z(n)/scaleheight/2)**2 +
!                    cosh(z(n)/scaleheight/2)**(-2)+1))/&
!                               (2*scaleheight**2*(tanh(z(n)/scaleheight/2)+1)**2)
!          aij_prof_z
!          =A*cosh(z(n)/scaleheight/2)**(-2)/((2*scaleheight)*tanh(z(n)/scaleheight/2)+2*scaleheight)
!          bij_prof_z = 2*A*sinh(z(n)/scaleheight)
          prof1 = A*4.0*scaleheight*atan(tanh(z(n)/scaleheight/2.0))
          do m=m1,m2
            
            imposed_b_field(l1:l2,n,1,aa_imp) =0! prof1
            imposed_b_field(l1:l2,n,2,bb_imp) =0! prof
            imposed_b_field(l1:l2,n,1,del2_imp) = 0!del2_prof
          end do
        end do
    call put_shared_variable('imposed_b_field',imposed_b_field)
      if(Pcr_to_Ptherm /= 0.) then
        ...
        do n=n1,n2
          ...
          gprof=-2*(A/scaleheight)*(cosh(z(n)/scaleheight))**(-2.0)*tanh(z(n)/scaleheight)
          do m=m1,m2
            imposed_cosmicray(l1:l2,m,n,ecr_imp)=prof
            imposed_cosmicray(l1:l2,m,n,gecr_imp) =gprof
          end do
        end do
    call put_shared_variable('imposed_cosmicray',imposed_cosmicray)
!
...
!
      ...
      ! Normalization of the profile
      f(l1:l2,m1:m2,n1:n2,ilnrho)= -100
